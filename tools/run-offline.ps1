<#
    run-smoke.ps1 â€” boot the real app in headless Chrome and report
...
if (-not $browser) { throw 'No Chrome or Edge found; cannot run the offline test.' }

$resultFile = Join-Path $env:TEMP 'pbc-offline-result.json'
$profileDir = Join-Path $env:TEMP 'pbc-offline-profile'
...
try {
    ...
    Write-Host "Running the offline test in $(Split-Path -Leaf $browser) ..." -ForegroundColor Cyan
    ...
    $chrome = Start-Process -FilePath $browser -PassThru -WindowStyle Hidden -ArgumentList @(
        ...
        "http://127.0.0.1:$Port/offlinetest.html"
    )
...
if (-not (Test-Path $resultFile)) {
    Write-Host 'NO RESULT: the offline test never finished.' -ForegroundColor Red
    exit 1
}
